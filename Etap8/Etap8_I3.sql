...
WHERE p.PenaltyID IN (
    SELECT /*+ INDEX(l IDX_LOAN_DATE_CONVERT) */ p2.PenaltyID
    FROM Penalties p2
    ...
    WHERE l.DueDate < TO_DATE('2022-01-01', 'YYYY-MM-DD')
      ...
      AND (
          SELECT COUNT(*) 
          FROM Loans l2
          WHERE /*+ INDEX(l2 IDX_LOAN_DATE_CONVERT) */ l2.ReaderID = l.ReaderID 
...
WHERE p.PenaltyID IN (
    ...
    WHERE l.DueDate < TO_DATE('2022-01-01', 'YYYY-MM-DD')
      ...
      AND p2.IsPaid = 'N'
      AND (
          SELECT /*+ INDEX(l2 IDX_LOAN_DATE_CONVERT) */ COUNT(*) 
          FROM Loans l2
