...
for reader in readers_data:
    reader_mapping[reader["ReaderID"]] =  datetime.datetime.strptime(reader["JoinDate"], "%Y-%m-%d %H:%M:%S").date()

book_mapping = {}
for book in books_data:
    book_mapping[book["BookID"]] = datetime.datetime.strptime(book["PublicationDate"], "%Y-%m-%d %H:%M:%S").date()
...
def generate_random_date(publication_date, join_date, end_year):
    ...
    latest_date = datetime.date(end_year, 12, 31)
    if earliest_date > latest_date:
        return earliest_date
    delta = latest_date - earliest_date
    random_days = random.randint(0, delta.days)
    return earliest_date + datetime.timedelta(days=random_days)
    ...
    for user_id in range(1, num_users):
        ...
        for _ in range(num_reviews):
            ...
            review = {
                ...
                "ReviewDate": str(generate_random_date(publication_date, join_date, end_year))
            }
...
for review in tqdm(reviews_data_copy, desc="Processing review spam generator", unit="review"):
    if review in selected_reviews:
        ...
        if random.randint(1, 5) == 2:
            review  = reviews_data[id]["ReviewText"]
            review += generate_random_link()
            reviews_data[id]["ReviewText"] = review
